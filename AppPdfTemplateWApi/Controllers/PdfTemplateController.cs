...
namespace AppPdfTemplateWApi.Controllers
{
    ...
    public class PdfTemplateController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetPredefinedTemplate(int showEventInfo, int ticketId)
        {
            if (showEventInfo < 1 || showEventInfo > 3)
            {
                return BadRequest("Template layout must be between 1 and 3.");
            }
            ...
            try
            {
                ...
                await _pdfTemplateService.CreatePdfAsync(outputPath, ticketData, ticketHandlingData, null);
